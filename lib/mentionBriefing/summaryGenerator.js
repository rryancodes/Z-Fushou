...
const SYSTEM_PROMPT = `You are a concise issue summarizer for a Discord community support channel.
Given a conversation context where a monitored user was mentioned, write a short issue brief.

Rules:
- 1-2 sentences maximum
- Describe what the user is asking or reporting
- Note whether it looks like a question, complaint, or request
- Mention which monitored user(s) were pinged if relevant
- Do NOT quote messages verbatim
- Do NOT add preamble like "The user is asking" — just state the issue directly
- Be specific about the topic (product names, features, error messages)
...
function buildSummaryPrompt(candidates, mentionedUserIds) {
  ...
  const mentionLabel = mentionedUserIds?.length
    ? `Monitored user IDs mentioned: ${mentionedUserIds.join(', ')}`
    : '';
