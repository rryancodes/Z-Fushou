...
const { getCredentials } = require('./cfCredentials');
...
// Cached OpenAI clients — one per shift (max 2 instances)
const clientCache = new Map();

/**
 * Returns an OpenAI SDK client for the given credential set.
 * Instances are cached per shift index to avoid re-creating on every call.
 */
function getClient(creds) {
  const key = creds.shift;
...
/**
 * Builds the native Cloudflare AI run URL for the current shift's account.
 * @param {string} model
 * @returns {string}
 */
...
async function embed(text) {
  ...
  const res = await fetchWithTimeout(runUrl(EMBEDDING_MODEL), {
    method: 'POST',
    headers: {
...
async function rerank(query, documents) {
  ...
  const res = await fetchWithTimeout(runUrl(RERANKER_MODEL), {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${apiToken}`,
      'Content-Type': 'application/json'
